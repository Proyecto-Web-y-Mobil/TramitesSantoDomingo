...
import {
  ...
  useIonToast, IonIcon, IonTextarea, IonItem, IonBadge, IonGrid, IonRow, IonCol
} from '@ionic/react';
import { useParams, useHistory } from 'react-router-dom';
import {
  documentOutline, checkmarkCircleOutline, alertCircleOutline,
...
export default function AdminRevisarTramite() {
  ...
  const cargarDetalle = async () => {
    try {
      const response = await fetch(
        `https://tramitessantodomingo-production-5cb4.up.railway.app/api/admin/tramites/${id}`
      );
  ...
  const actualizarEstado = async (nuevoEstado: string) => {
    ...
    try {
      const response = await fetch(
        `https://tramitessantodomingo-production-5cb4.up.railway.app/api/admin/tramites/${id}/estado`,
        {
          method: 'PUT',
