...
import {
  ...
  useIonToast, IonSelect, IonSelectOption, IonItem, IonInput
} from '@ionic/react';
...
export default function ReportesAdmin() {
  ...
  const cargarListaTalleres = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3000/api/admin/reportes/talleres');
  ...
  const generarReporteDideco = async (idTaller: string) => {
    try {
      // 🔥 CORRECCIÓN: Apuntando al backend local
      const response = await fetch(`http://localhost:3000/api/admin/reportes/taller/${idTaller}`);
      const data = await response.json();
  ...
  const generarReporteTransito = async (fecha: string) => {
    try {
      // 🔥 CORRECCIÓN: Apuntando al backend local
      const response = await fetch(`http://localhost:3000/api/admin/reportes/transito/${fecha}`);
      const data = await response.json();
